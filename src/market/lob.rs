...
/// A price level holds orders in FIFO insertion order.
///
/// `queue` contains order IDs in arrival order and may include "holes" —
/// IDs of orders that were cancelled or expired. Holes are cleaned up lazily
/// during matching (pop_front skips any ID not present in `orders`).
///
/// This gives O(1) cancel-by-id (`orders.remove`) vs the previous O(n) retain.
pub struct PriceLevel {
    queue: VecDeque<u64>,
    orders: FxHashMap<u64, LobOrder>,
...
impl PriceLevel {
    ...
    fn push(&mut self, order: LobOrder) {
        self.queue.push_back(order.order_id);
        self.total_quantity += order.quantity;
        self.orders.insert(order.order_id, order);
    }

    /// Remove a specific order by id in O(1). Leaves a hole in the queue
    /// that will be skipped during matching.
    fn remove_by_id(&mut self, order_id: u64) -> Option<LobOrder> {
...
/// Sentinel tick value for persistent MM quotes.
...
pub struct LimitOrderBook {
    ...
    // Reusable scratch buffers — cleared and refilled each call.
...
impl LimitOrderBook {
            ...
            for order_id in order_ids {
                if let Some((side, price)) = self.order_index.remove(&order_id) {
                    ...
                    if let Some(level) = book_side.levels.get_mut(&price) {
                        level.remove_by_id(order_id); // O(1)
                        if level.is_empty() {
    ...
    fn submit_order(&mut self, mut order: LobOrder, trades_out: &mut Vec<Trade>) {
        ...
        if trades_out.len() > before {
            self.last_price = trades_out.last().unwrap().price;
        }
    ...
    fn match_buy_order(&mut self, order: &mut LobOrder, trades: &mut Vec<Trade>) {
        ...
        for (&price, level) in self.asks.levels.iter_mut() {
            if order.quantity <= f32::EPSILON { break; }
            if !is_market && price.into_inner() > order.price { break; }

            while order.quantity > f32::EPSILON && !level.is_empty() {
                // Drain holes (cancelled/expired IDs) from the front of the queue.
                while level.queue.front().map_or(false, |&oid| !level.orders.contains_key(&oid)) {
                    level.queue.pop_front();
                }
                let front_id = match level.queue.front().copied() {
                    Some(oid) => oid,
                    None => break,
                };

                let (fill_qty, seller_id, is_done) = {
                    let resting = level.orders.get_mut(&front_id).unwrap();
                    let fq = order.quantity.min(resting.quantity);
                    resting.quantity -= fq;
                    (fq, resting.agent_id, resting.quantity <= f32::EPSILON)
                };
                ...
                if is_done {
                    level.queue.pop_front();
                    let done = level.orders.remove(&front_id).unwrap();
        ...
        for i in 0..self.filled_resting_buf.len() {
            ...
            if let Some(ids) = self.agent_orders.get_mut(&agent_id) {
    ...
    fn rest_order(&mut self, order: LobOrder) {
        ...
        let level = book_side.levels.entry(price).or_insert_with(PriceLevel::new);
        level.push(order);
    ...
    /// Trades are appended to `trades_out` (caller clears before calling).
    pub fn process_tick(
        &mut self,
        cancel_agents: &[u32],
    ...
    fn expire_side(book_side: &mut BookSide, min_tick: u64) -> usize {
        ...
        for (&price, level) in book_side.levels.iter_mut() {
            let mut removed_qty = 0.0_f32;
            level.orders.retain(|_, o| {
                if o.tick < min_tick {
                    removed_qty += o.quantity;
                    removed += 1;
                    false
                } else {
                    true
    ...
    pub fn bbo(&self) -> BBO {
        let half_spread = self.last_price * 0.001;
        let (best_bid, best_bid_size) = self.bids.levels.iter().next_back()
            .map(|(&p, l)| (p.into_inner(), l.total_quantity))
            .unwrap_or((self.last_price - half_spread, 0.0));
        let (best_ask, best_ask_size) = self.asks.levels.iter().next()
            .map(|(&p, l)| (p.into_inner(), l.total_quantity))
            .unwrap_or((self.last_price + half_spread, 0.0));
        BBO {
            best_bid,
    ...
    pub fn book_bids(&self, n: usize) -> Vec<(f32, f32)> {
        self.bids.levels.iter().rev().take(n)
            .map(|(&p, l)| (p.into_inner(), l.total_quantity))
            .collect()
    }

    pub fn book_asks(&self, n: usize) -> Vec<(f32, f32)> {
        self.asks.levels.iter().take(n)
            .map(|(&p, l)| (p.into_inner(), l.total_quantity))
            .collect()
    }
}
...
mod tests {
    ...
    fn limit_buy(agent_id: u32, price: f32, qty: f32, tick: u64) -> LobOrder {
        LobOrder { order_id: 0, agent_id, side: Side::Buy, price, quantity: qty,
                   order_type: OrderType::Limit, tick }
    }
    fn limit_sell(agent_id: u32, price: f32, qty: f32, tick: u64) -> LobOrder {
        LobOrder { order_id: 0, agent_id, side: Side::Sell, price, quantity: qty,
                   order_type: OrderType::Limit, tick }
    }
    fn market_buy(agent_id: u32, qty: f32, tick: u64) -> LobOrder {
        LobOrder { order_id: 0, agent_id, side: Side::Buy, price: f32::MAX, quantity: qty,
                   order_type: OrderType::Market, tick }
    }
    fn market_sell(agent_id: u32, qty: f32, tick: u64) -> LobOrder {
        LobOrder { order_id: 0, agent_id, side: Side::Sell, price: 0.0, quantity: qty,
                   order_type: OrderType::Market, tick }
    }
    ...
    fn test_expire_preserves_newer() {
        let mut book = LimitOrderBook::new(100.0);
        let mut trades = Vec::new();
        book.process_tick(&[], vec![], vec![limit_buy(0, 99.0, 5.0, 0)], 0, &mut trades);
        trades.clear();
        book.process_tick(&[], vec![], vec![limit_sell(1, 101.0, 3.0, 1)], 1, &mut trades);
        assert_eq!(book.book_depth(), 2);
        book.expire_orders_before(1);
        assert_eq!(book.book_depth(), 1);
    ...
    fn test_persistent_orders_survive_expire() {
        ...
        book.process_tick(&[], vec![], vec![
            limit_buy(0, 99.0, 5.0, 0),
            limit_sell(1, 101.0, 3.0, PERSISTENT_TICK),
        ], 0, &mut trades);
        assert_eq!(book.book_depth(), 2);
        book.expire_orders_before(1);
        assert_eq!(book.book_depth(), 1);
    ...
    /// Many agents at the same price level — cancel must be O(1) regardless of depth.
    #[test]
    fn test_cancel_at_crowded_level() {
        let mut book = LimitOrderBook::new(100.0);
        for agent in 0..100_u32 {
            book.submit_order_vec(limit_sell(agent, 101.0, 1.0, PERSISTENT_TICK));
        }
        assert_eq!(book.book_depth(), 100);
        book.cancel_agent(42);
        assert_eq!(book.book_depth(), 99);
        book.cancel_agent(0);
        book.cancel_agent(99);
        assert_eq!(book.book_depth(), 97);
    }

    /// After cancellation, holes in the queue must not affect matching correctness.
    #[test]
    fn test_match_after_cancel_leaves_holes() {
        let mut book = LimitOrderBook::new(100.0);
        book.submit_order_vec(limit_sell(0, 100.0, 2.0, PERSISTENT_TICK));
        book.submit_order_vec(limit_sell(1, 100.0, 2.0, PERSISTENT_TICK));
        book.submit_order_vec(limit_sell(2, 100.0, 2.0, PERSISTENT_TICK));
        // Cancel the middle one — leaves a hole in the queue
        book.cancel_agent(1);
        assert_eq!(book.book_depth(), 2);
        // Buy 3: should match agent 0 (2 units) then agent 2 (1 unit), skipping hole
        let trades = book.submit_order_vec(limit_buy(99, 100.0, 3.0, 0));
        assert_eq!(trades.len(), 2);
        assert_eq!(trades[0].seller_id, 0);
        assert_eq!(trades[0].quantity, 2.0);
        assert_eq!(trades[1].seller_id, 2);
